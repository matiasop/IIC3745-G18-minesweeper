  ...
  def mark_contiguous
    @n_rows.times do |row|
      @n_cols.times do |col|
        ...
        if row.positive? && col.positive?
          @board["(#{row}, #{col})"].contiguous['NW'] =
            @board["(#{row - 1}, #{col - 1})"]
        end

        if row.positive? && col < @n_cols - 1
          @board["(#{row}, #{col})"].contiguous['NE'] = @board["(#{row - 1}, #{col + 1})"]
        end

        if row < @n_rows - 1 && col.positive?
  ...
  def print_ingame_board
    @n_rows.times do |row|
      @n_cols.times do |col|
        actual = @board["(#{row}, #{col})"]
        if actual.played
          if actual.n_contiguous_bombs.positive?
            print "[ #{actual.n_contiguous_bombs} ] "
          else
            print '[ - ] '
          end
  ...
  def print_hidden_board
    @n_rows.times do |row|
      @n_cols.times do |col|
        ...
        if actual.bomb
          print '[ * ] '
        elsif actual.n_contiguous_bombs.positive?
          print "[ #{actual.n_contiguous_bombs} ] "
        else
          print '[ - ] '
        end
